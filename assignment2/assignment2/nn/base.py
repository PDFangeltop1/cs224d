...
class PackedVector(object):
    _name_to_idx = {}  # store parameter blocks.
    _views = []

    def __init__(self, *shapes, **shapemap):
        # *shapes : list of arguments ,not know the length of it.
        # ** shapemap : key-value pairs
        ...
        #print self._name_to_idx
        # Generate endpoints
        self._dims = shapes
        self._lens = map(np.prod, self._dims)
        self._ends = concatenate([[0], cumsum(self._lens)])
        self._vec = zeros(self._ends[-1]) # allocate storage ,store all params

        # Generate view objects
        # separate each paramter blocks
        # _views[0] --> W1
        # _views[1] --> b1
        # _views[2] --> W2
        # _views[3] --> b2 etc.
...
class NNBase(object):
    ...
    def train_sgd(self, X, y,
                  ...
                  printevery=10000, costevery=4000,
                  devidx=None):
